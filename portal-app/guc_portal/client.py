...
class GucPortal:
    ...
    def available_seasons(self) -> list[tuple[str, str]]:
        ...
        return parse_options(html, SEASON_SUFFIX)
    ...
    def list_previous_courses(self, season_value: str) -> list[tuple[str, str]]:
        ...
        after_season = self._postback(url, html, _field_name(html, SEASON_SUFFIX), season_value)
    ...
    def get_previous_grades(
        ...
        for _ in range(tries):
            try:
                html, url = self._get(self.site.prev_grades_path)
                season_field = _field_name(html, SEASON_SUFFIX)
                ...
                season_label = dict(parse_options(html, SEASON_SUFFIX)).get(season_value)
    ...
    def get_grades_by_name(
        ...
        for _ in range(tries):
            try:
                html, url = self._get(self.site.prev_grades_path)
                seasons = parse_options(html, SEASON_SUFFIX)
